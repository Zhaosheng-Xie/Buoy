...
Thanks for Hao Shen. He helped us a lot no matter in data reading, tidying and analysing. 
...
In analysis process, we will use two variables WTMP *(Water Temperature)* and ATMP *(Air Temperature)* respectively. So, here we need to create Bouy_W and Buoy_A data frames without NA values (denoted by 99.0 or 999.0) for WTMP and analysis respectively. In this step, we only delete 1,476 and 2,298 data points from Buoy_W and Buoy_A which are less than 1%.
...
for(i in 1988:2015){
  ...
  #                  )
  #This note above also does the same job as code hereafter, but the latter one is more compact.You will find same improvements in other places.
...
for(j in 1:12){
  ...
  for(i in 1988:2015){
    ...
    #                                ifelse(j==12,1,j+1))])
    #This note above also does the same job as code hereafter, but the latter one is more compact.
